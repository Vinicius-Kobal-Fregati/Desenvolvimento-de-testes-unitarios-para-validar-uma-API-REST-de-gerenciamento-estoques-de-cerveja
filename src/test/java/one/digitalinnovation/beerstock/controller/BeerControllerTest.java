...
@ExtendWith(MockitoExtension.class) // Estamos estendendo a classe do mockito.
public class BeerControllerTest {
    ...
    @InjectMocks // Aqui vamos injetar um mock para nossa beerService
    private BeerController beerController;

    @BeforeEach // Antes de cada teste, faremos esses processos.
    void setUp() {
        mockMvc = MockMvcBuilders.standaloneSetup(beerController) // Vamos fazer o setup apenas para a classe beerController.
                .setCustomArgumentResolvers(new PageableHandlerMethodArgumentResolver()) // Vai gerar suporte a objetos pagináveis.
                .setViewResolvers((s, locale) -> new MappingJackson2JsonView()) // Vamos transformar Jackson para Json (assim vamos simular o retorno de uma API).
                .build();
    }

    @Test
    void whenPOSTIsCalledThenABeerIsCreated() throws Exception { // Cria uma cerveja quando o método Post é chamado.
        // given
        BeerDTO beerDTO = BeerDTOBuilder.builder().build().toBeerDTO(); // Vamos ter nosso BeerDTO.
        ...
        mockMvc.perform(post(BEER_API_URL_PATH)
                ...
                .andExpect(status().isCreated()) // Vamos aguardar o status de criação.
                .andExpect(jsonPath("$.name", is(beerDTO.getName()))) // Vamos esperar o dado nome.
                .andExpect(jsonPath("$.brand", is(beerDTO.getBrand()))) // Vamos esperar o dado marca.
                .andExpect(jsonPath("$.type", is(beerDTO.getType().toString()))); // Vamos esperar o dado tipo.
    }

    @Test
    void whenPOSTIsCalledWithoutRequiredFieldThenAnErrorIsReturned() throws Exception { // Quando ocorre uma chamada post sem um campo obrigatório, é retornado um erro.
        // given
        BeerDTO beerDTO = BeerDTOBuilder.builder().build().toBeerDTO();
        beerDTO.setBrand(null); // Tiramos a marca da cerveja.

        // then
        mockMvc.perform(post(BEER_API_URL_PATH)
                .contentType(MediaType.APPLICATION_JSON)
                .content(asJsonString(beerDTO)))
                .andExpect(status().isBadRequest()); // Espera o erro de requisição errada (400).
    }
